...
- 例句可加 `"noQuiz": true`，代表只用來講解、不拿去出題（數字列表打散重排沒有意義）

**練習題會自己長出來，不需要手寫題庫。** 單字題由 `vocab` 組合；助詞填空與排列組句
則是把 `examples` 丟給 kuromoji 斷詞後自動生成，所以同一課每次練到的題目都不一樣。

單字題型是針對**中文母語者**設計的。看到漢字就知道意思是母語優勢，但讀音是死穴，
所以題目一律不同時給漢字和假名：

| 題型 | 題目 | 選項 | 對應 N5 |
|---|---|---|---|
| 漢字 → 讀音 | `先生`（不標假名） | がくせい／せんせい／… | 漢字読み |
| 假名 → 漢字 | 「せんせい」 | 学生／先生／… | 表記 |
| 假名 → 中文 | `せんせい`（不給漢字） | 老師／學生／… | — |
| 中文 → 假名 | 「老師」 | せんせい／がくせい／… | — |

干擾項會刻意挑「像」的：假名長度相近、或共用同一個漢字（問 にほんじん 時放 台湾人），
否則一眼就能刪掉。

課程單字會在啟動時登記給斷詞器（`registerWords`）。kuromoji 的字典不認得的複合詞
（台湾人、勉強します）會被切碎，登記後會併回一個詞，並直接採用課程寫的讀音——
所以**只要單字表寫對，例句和練習題裡的讀音就一定跟它一致**。
